...
function getSiteData(url, state) {
  ...
  let currentCity = currentState.stores.find(city => city.url === url.toLowerCase())

  // 3. Create a siteData object
  const siteData = {
    "url": `https://milanlaser${currentCity.url.toLowerCase()}.com/`,
